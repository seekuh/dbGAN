...
def display(tfrecord_dir):
    ...
    while True:
        ...
        plt.imshow(images[0].transpose(1, 2, 0))  # CHW => HWC, RGB => BGR
        idx += 1
        #
        # if cv2.waitKey() == 27:
        #     break
...
# ----------------------------------------------------------------------------
def create_cifar10_subset(tfrecord_dir, cifar10_dir, subset_ids):
    print('Loading CIFAR-10 from "%s"' % cifar10_dir)
    print('Subset indices: ', str(subset_ids))
    import pickle
    images = []
    labels = []
    for batch in subset_ids:
        with open(os.path.join(cifar10_dir, 'data_batch_%d' % batch), 'rb') as file:
            data = pickle.load(file, encoding='latin1')
        images.append(data['data'].reshape(-1, 3, 32, 32))
        labels.append(data['labels'])
    ...
    print('Population size: %d' % len(images))
    with TFRecordExporter(tfrecord_dir, images.shape[0]) as tfr:
        order = tfr.choose_shuffled_order()
        for idx in range(order.size):
            tfr.add_image(images[order[idx]])
...
def create_celeba_subset(tfrecord_dir, celeba_dir, cx=89, cy=121):
    print('Loading CelebA from "%s"' % celeba_dir)
    glob_pattern = os.path.join(celeba_dir, '*.png')
    image_filenames = sorted(glob.glob(glob_pattern))
    print('Dataset size: %d' % len(image_filenames))
...
def create_from_npy(tfrecord_dir, npy_filepath, shuffle):
    print('Loading images from "%s"' % npy_filepath)

    imgs = np.load(npy_filepath)
    resolution = imgs.shape[1]
    channels = imgs.shape[-1]
    print('Dataset size: %d' % len(imgs))

    if imgs.shape[2] != resolution:
    ...
    with TFRecordExporter(tfrecord_dir, len(imgs)) as tfr:
        ...
        for idx in range(order.size):
            img = np.rint(imgs[order[idx]] * 255)
            if channels == 1:
...
def execute_cmdline(argv):
    ...
    p = add_command('create_celeba_subset', 'Create dataset for CelebA subset.',
                    'create_celeba_subset datasets/celeba/train ~/downloads/celeba')
    p.add_argument('tfrecord_dir', help='New dataset directory to be created')
    p.add_argument('celeba_dir', help='Directory containing CelebA')
    p.add_argument('--cx', help='Center X coordinate (default: 89)', type=int, default=89)
    p.add_argument('--cy', help='Center Y coordinate (default: 121)', type=int, default=121)
    ...
    p = add_command('create_from_npy', 'Create dataset from npy file.',
                    'create_from_npy datasets/mydataset mynpyfile')
    p.add_argument('tfrecord_dir', help='New dataset directory to be created')
    p.add_argument('npy_filepath', help='Path of the .npy file')
    p.add_argument('--shuffle', help='Randomize image order (default: 1)', type=int, default=1)

    p = add_command('create_from_npz', 'Create dataset from npz file.',
                    'create_from_npz datasets/mydataset mynpzfile')
    p.add_argument('tfrecord_dir', help='New dataset directory to be created')
    p.add_argument('npz_filepath', help='Path of the .npz file')
    p.add_argument('--shuffle', help='Randomize image order (default: 1)', type=int, default=1)
